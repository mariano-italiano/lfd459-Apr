...
HARBORFILE=$(ls harbor-offline*)
tar -xzvf /tmp/$HARBORFILE
sudo mv harbor /opt/
...
### Trust Harbar certificate on Docker Host

```bash
# upload to docker
sudo mkdir /etc/docker/certs.d/registry.lab.local"
scp student@registry.lab.local:/opt/harbor/ca.crt /etc/docker/certs.d/registry.lab.local/"

# upload to ca-certificates
scp student@registry.lab.local:/opt/harbor/ca.crt /usr/local/share/ca-certificates/harbor-ca.crt
